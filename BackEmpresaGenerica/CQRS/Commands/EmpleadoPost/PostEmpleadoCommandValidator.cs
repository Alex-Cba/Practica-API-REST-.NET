...
namespace EmpresaGenericaAPI.CQRS.Commands.EmpleadoPost
{
    public class PostEmpleadoCommandValidator : AbstractValidator<PostEmpleadoCommand>
    {
        ...
        public PostEmpleadoCommandValidator(EfcodeFirstContext context)
        {
            ...
            RuleFor(rule => rule)
                .MustAsync(CheckEmpleado).WithMessage("El empleado ya existe");
            RuleFor(rule => rule)
        ...
        private async Task<bool> CheckEmpleado(PostEmpleadoCommand command, CancellationToken Token)
        {
            bool CheckEmpleado = await _context.Empleados.AnyAsync(p => p.DNI == command.DNI);
            return !CheckEmpleado;
        }

        private async Task<bool> CheckCargos(PostEmpleadoCommand command, CancellationToken Token)
        {
            bool CheckCargos = await _context.Cargos.AnyAsync(p => p.Id == command.CargoId);
            return CheckCargos;
        }

        private async Task<bool> CheckSucursales(PostEmpleadoCommand command, CancellationToken Token)
        {
            bool CheckSucursales = await _context.Sucursales.AnyAsync(p => p.Id == command.SucursalId);
            return CheckSucursales;
        }

        private async Task<bool> CheckJefes(PostEmpleadoCommand command, CancellationToken Token)
        {
